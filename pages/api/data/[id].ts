...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case 'GET':
      try {
        const querys = `SELECT data.*,program.name as program_name, train_history.status as status  from data left join program on program.id = data.program_id LEFT JOIN train on data.train_id = train.id left join train_history on train_history.train_id = train.id WHERE data.program_id = ${query.id} AND data.is_deleted = FALSE order by data.id`;
