      ...
      >
        <Box sx={{ height: '100%' }} display={'flex'} flexDirection={'row'}>
          ...
          >
            <Box margin={2}>
              <Autocomplete
                ...
                onChange={(event: any, newValue: any) => {
                  console.log('**** autocomplete', typeof event, newValue);
                  setCurrentUser(newValue?.id);
                }}
                renderInput={(params) => (
            ...
            <Scrollbar autoScrollBottom={false}>
              <SidebarContent
                selectedId={currentUser?.id}
                userList={userList}
                onClick={(user) => setCurrentUser(user)}
              />
            </Scrollbar>
          </Sidebar>
                ...
                {paginatedhisotryDatas.map((item) => {
                  ...
                  return (
                    <TableRow
                      hover
                      key={item.id}
                      selected={ishisotryDataselected}
                    >
